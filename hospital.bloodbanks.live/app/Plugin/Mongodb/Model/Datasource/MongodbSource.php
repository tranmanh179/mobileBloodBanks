		...
		try{
			...
			if (isset($this->config['replicaset']) && count($this->config['replicaset']) === 2) {
				...
				try{
					...
					echo 'Lỗi kết nối: '.$_SERVER['SERVER_ADDR'];
					$this->error = $e->getMessage();
					trigger_error($this->error);
					die;
				}
 ...
 */
		public function createConnectionName($config, $version) {
			...
			if(!empty($config['login'])){
				...
				$param= array();
				if(!empty($config['ssl'])) $param[]=  'ssl='.$config['ssl'];
				if(!empty($config['replicaSetMantan'])) $param[]=  'replicaSet='.$config['replicaSetMantan'];
				if(!empty($config['authSource'])) $param[]=  'authSource='.$config['authSource'];
				if(!empty($config['retryWrites'])) $param[]=  'retryWrites='.$config['retryWrites'];
